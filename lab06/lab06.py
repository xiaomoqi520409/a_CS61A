...
class Server:
    ...
    dictionary from client names to client objects.每个服务器都有一个实例属性,名为clients,它是一个从客户端名称到客户端对象的字典。
    """
    def __init__(self):
        self.clients = {}

    def send(self, email):
        """Append the email to the inbox of the client it is addressed to. 将电子邮件添加到收件客户的收件箱中。"""
        self.clients[email.recipient_name].inbox.append(email)

    def register_client(self, client):
        """Add a client to the dictionary of clients. 将一个客户添加到客户字典中。"""
        self.clients[client.name] = client

class Client:
    """A client has a server, a name (str), and an inbox (list).  一个客户端有一个服务器、一个名称（字符串）和一个收件箱（列表）。
    ...
    def compose(self, message, recipient_name):
        """Send an email with the given message to the recipient. 使用给定的消息向收件人发送一封电子邮件"""
...
def make_change(amount, coins):
    ...
    返回一个硬币列表，这些硬币的总和等于目标金额，优先选择最小的可用硬币，并将最小的硬币放在返回列表的首位。
    The coins argument is a dictionary with keys that are positive integer
    denominations and values that are positive integer coin counts.
    coins参数是一个字典，其键为正整数面额，值为正整数硬币数量。
    >>> make_change(2, {2: 1})
    [2]
